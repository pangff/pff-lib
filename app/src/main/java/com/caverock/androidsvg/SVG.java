 ...
 * </pre>
 *
 * For more detailed information on how to use this library, see the documentation at {@code
...
public class SVG {
    ...
    /**
     * Register an {@link SVGExternalFileResolver} instance that the renderer should use when
     * resolving
     ...
     * conversion of "physical" units - such an "pt" or "cm" - to pixel values.  The default DPI is
     * 96.
     * <p>
     * You should not normally need to alter the DPI from the default of 96 as recommended by the
     * SVG
     ...
     * <p>
     * An attempt will be made to determine a suitable initial viewport from the contents of the SVG
     * file.
    ...
    /**
     * Renders this SVG document to a Picture object using the specified view defined in the
     * document.
     * <p>
     * A View is an special element in a SVG document that describes a rectangular area in the
     * document.
     * Calling this method with a {@code viewId} will result in the specified view being positioned
     * and scaled
     * to the viewport.  In other words, use {@link #renderToPicture()} to render the whole
     * document, or use this
     * method instead to render just a part of it.
     *
     * @param viewId         the id of a view element in the document that defines which section of
     *                       the document is to be visible.
     * @param widthInPixels  the width of the initial viewport
     * @param heightInPixels the height of the initial viewport
     * @return a Picture object suitable for later rendering using {@code Canvas.drawPicture()}, or
     * null if the viewId was not found.
     */
     ...
     *
     * @param canvas   the canvas to which the document should be rendered.
     * @param viewPort the bounds of the area on the canvas you want the SVG rendered, or null for
     *                 the whole canvas.
     */
    public void renderToCanvas(Canvas canvas, RectF viewPort) {
        ...
        if (viewPort != null) {
            svgViewPort = Box
                    .fromLimits(viewPort.left, viewPort.top, viewPort.right, viewPort.bottom);
        } else {
     ...
     * <p>
     * A View is an special element in a SVG documents that describes a rectangular area in the
     * document.
     * Calling this method with a {@code viewId} will result in the specified view being positioned
     * and scaled
     * to the viewport.  In other words, use {@link #renderToPicture()} to render the whole
     ...
     *
     * @param viewId the id of a view element in the document that defines which section of the
     *               document is to be visible.
     * @param canvas the canvas to which the document should be rendered.
     */
     ...
     *
     * @return the document's viewBox attribute as a {@code android.graphics.RectF} object, or null
     * if not set.
     * @throws IllegalArgumentException if there is no current SVG document loaded.
     */
     ...
     *
     * @param preserveAspectRatio the new {@code preserveAspectRatio} setting for the root {@code
     *                            <svg>} element.
     * @throws IllegalArgumentException if there is no current SVG document loaded.
     */
    ...
    private Box getDocumentDimensions(float dpi) {
        ...
        if (w == null || w.isZero() || w.unit == Unit.percent || w.unit == Unit.em
                || w.unit == Unit.ex) {
    ...
    protected static class Box implements Cloneable {
        ...
        public void union(Box other) {
            ...
            if (other.maxX() > maxX()) {
                width = other.maxX() - minX;
            }
            if (other.maxY() > maxY()) {
                height = other.maxY() - minY;
            }
        }
    ...
    protected static class Colour extends SvgPaint {
        ...
        public static final Colour BLACK = new Colour(0);
                // Black singleton - a common default value.
            ...
            for (int commandPos = 0; commandPos < commandsLength; commandPos++) {
                ...
                switch (command) {
                    ...
                    case LINETO:
                        ...
                        break;
                    case CUBICTO:
                        handler.cubicTo(coords[coordsPos++], coords[coordsPos++],
                                coords[coordsPos++], coords[coordsPos++], coords[coordsPos++],
                                coords[coordsPos++]);
                        break;
                    case QUADTO:
                        handler.quadTo(coords[coordsPos++], coords[coordsPos++],
                                coords[coordsPos++], coords[coordsPos++]);
                        break;
                    case CLOSE:
                        handler.close();
                        break;
