 ...
 * Description MyPathView
 */
public class MyPathView extends View implements MySvgUtils.AnimationStepListener {
     ...
     */
    public final MySvgUtils mySvgUtils = new MySvgUtils(paint);

    /**
     * All the paths provided to the view. Both from Path and Svg.
     */
    private LinkedHashMap<Integer, MySvgUtils.SvgPath> paths = new LinkedHashMap<>();
    ...
    private void getFromAttributes(Context context, AttributeSet attrs) {
        ...
        try {
            if (a != null) {
                ...
                svgResourceId = a.getResourceId(R.styleable.PathView_svgRes, 0);
    ...
    public void setPaths(final List<Path> paths) {
        for (Path path : paths) {
            this.paths.put(path.hashCode(), new MySvgUtils.SvgPath(MySvgUtils.SvgPath.TYPE_PATH,"", path, paint));
        }
     ...
     * Set paths to be drawn and animated.
     *
     * @param  - Paths that can be drawn.
     */
    public void setSvgDrawPaths(final List<String> pathIds) {
    ...
    public void setPath(final Path path) {
        paths.put(path.hashCode(), new MySvgUtils.SvgPath(MySvgUtils.SvgPath.TYPE_PATH,"", path, paint));
        synchronized (mSvgLock) {
            updatePathsPhaseLocked();
        }
    }
    ...
    private void updatePathsPhaseLocked() {
        Iterator<Integer> keySetIt = paths.keySet().iterator();
        while (keySetIt.hasNext()) {

            MySvgUtils.SvgPath svgPath = paths.get(keySetIt.next());
            if(isDraw(svgPath.id)){
                svgPath.path.reset();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mTempBitmap == null || (mTempBitmap != null && (
                mTempBitmap.getWidth() != canvas.getWidth() || mTempBitmap.getHeight() != canvas
                        .getHeight()))) {
            mTempBitmap = Bitmap
        ...
        synchronized (mSvgLock) {
            ...
            if(pathIds.size()>0){
                mTempCanvas.drawColor(Color.parseColor("#b0ffffff"));
            }
...
//            fillAfter(mTempCanvas);
    ...
    private void fillAfter(final Canvas canvas) {
        if (svgResourceId != 0 && fillAfter && progress == 1f) {
            mySvgUtils.drawSvgAfter(canvas, width, height);
        }
    }
    ...
    public interface SvgLoadedListener{
       void onSuccess();
    }
    SvgLoadedListener svgLoadedListener;
    public void setSvgLoadedListener(SvgLoadedListener svgLoadedListener){
        this.svgLoadedListener = svgLoadedListener;
    }
        ...
        if (svgResourceId != 0) {
            mLoader = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    mySvgUtils.load(getContext(), svgResourceId);

                    synchronized (mSvgLock) {
                        width = w - getPaddingLeft() - getPaddingRight();
                        height = h - getPaddingTop() - getPaddingBottom();
                        paths = mySvgUtils.getPathsForViewport(width, height);
                        updatePathsPhaseLocked();
                        if(svgLoadedListener!=null){
                            svgLoadedListener.onSuccess();
                        }
                    }
                }
            }, "SVG Loader");
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Iterator<Integer> keySetIt = paths.keySet().iterator();
        while (keySetIt.hasNext()) {
            MySvgUtils.SvgPath path = paths.get(keySetIt.next());
    ...
    public static class AnimatorBuilder {
         ...
         */
        public AnimatorBuilder(final MyPathView pathView) {
    ...
    public static class AnimatorSetBuilder {
         ...
         */
        private LinkedHashMap<Integer, MySvgUtils.SvgPath> paths;
         ...
         * @param pathView The view that must be animated.
         */
        public AnimatorSetBuilder(final MyPathView pathView) {
            ...
            while (keySetIt.hasNext()) {
                ...
                ObjectAnimator animation = ObjectAnimator
                        .ofFloat(path, "length", 0.0f, path.getLength());
    ...
    private  MySvgUtils.SvgPath findClickObject(int x,int y){
        Iterator<Integer> keySetIt = paths.keySet().iterator();
        while (keySetIt.hasNext()) {
            MySvgUtils.SvgPath svgPath = paths.get(keySetIt.next());
            if(svgPath.bounds.contains(x,y)){
                return svgPath;
            }
        }
        return null;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                MySvgUtils.SvgPath svgPath = findClickObject((int)event.getX(),(int)event.getY());
                if(svgPath!=null){
                    Toast.makeText(this.getContext(),"点击:"+svgPath.id,Toast.LENGTH_SHORT).show();
                }
                break;
