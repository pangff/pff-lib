...
public class SVGParser extends DefaultHandler2 {
    ...
    private static final String VALID_DISPLAY_VALUES =
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
        ...
        if (currentElement instanceof SVG.TextContainer) {
            ...
            SVG.SvgObject previousSibling = (numOlderSiblings == 0) ? null
                    : parent.children.get(numOlderSiblings - 1);
                ...
                case pathLength:
                    ...
                    if (obj.pathLength < 0f) {
                        throw new SAXException(
                                "Invalid <path> element. pathLength cannot be negative");
                    }
                    break;
                default:
                    break;
    ...
    private void parseAttributesEllipse(SVG.Ellipse obj, Attributes attributes)
            throws SAXException {
        for (int i = 0; i < attributes.getLength(); i++) {
            String val = attributes.getValue(i).trim();
            switch (SVGAttr.fromString(attributes.getLocalName(i))) {
    ...
    */
                ...
                while (!scan.empty()) {
                    ...
                    if (Float.isNaN(x)) {
                        throw new SAXException("Invalid <" + tag
                                + "> points attribute. Non-coordinate content found in list.");
                    }
        ...
        for (int i = 0; i < attributes.getLength(); i++) {
            ...
            switch (SVGAttr.fromString(attributes.getLocalName(i))) {
                ...
                case requiredFonts:
                    ...
                    Set<String> fontSet = (fonts != null) ? new HashSet<String>(fonts)
            ...
            switch (SVGAttr.fromString(attributes.getLocalName(i))) {
                ...
                case spreadMethod:
                    try {
                        ...
                        throw new SAXException("Invalid spreadMethod attribute. \"" + val
        ...
        for (int i = 0; i < attributes.getLength(); i++) {
            ...
            switch (SVGAttr.fromString(attributes.getLocalName(i))) {
                ...
                case r:
                    ...
                    if (obj.r.isNegative()) {
                        throw new SAXException(
                                "Invalid <radialGradient> element. r cannot be negative");
                    }
                    break;
                case fx:
    ...
    private void parseAttributesPattern(SVG.Pattern obj, Attributes attributes)
            throws SAXException {
        for (int i = 0; i < attributes.getLength(); i++) {
            String val = attributes.getValue(i).trim();
            switch (SVGAttr.fromString(attributes.getLocalName(i))) {
                ...
                case width:
                    ...
                    if (obj.width.isNegative()) {
                        throw new SAXException(
                                "Invalid <pattern> element. width cannot be negative");
                    }
                    break;
                case height:
                    obj.height = parseLength(val);
                    if (obj.height.isNegative()) {
                        throw new SAXException(
                                "Invalid <pattern> element. height cannot be negative");
                    }
                    break;
                case href:
                    if (!XLINK_NAMESPACE.equals(attributes.getURI(i))) {
    ...
    protected static class TextScanner {
       ...
       */
        ...
        public boolean consume(String str) {
            int len = str.length();
            boolean found = (position <= (inputLength - len) && input
                    .substring(position, position + len).equals(str));
            if (found) {
                position += len;
            }
            return found;
       ...
       */
        ...
        public Unit nextUnit() {
            ...
            try {
                Unit result = Unit
                        .valueOf(input.substring(position, position + 2).toLowerCase(Locale.US));
                position += 2;
                return result;
            } catch (IllegalArgumentException e) {
                return null;
    ...
    */
    private static void parseStyle(SvgElementBase obj, String style) throws SAXException {
        TextScanner scan = new TextScanner(
                style.replaceAll("/\\*.*?\\*/", ""));  // regex strips block comments
    ...
    protected static void processStyleProperty(Style style, String localName, String val)
        ...
        switch (SVGAttr.fromString(localName)) {
            ...
            case marker:
                ...
                style.specifiedFlags |= (SVG.SPECIFIED_MARKER_START | SVG.SPECIFIED_MARKER_MID
                        | SVG.SPECIFIED_MARKER_END);
                break;
            ...
            case visibility:
                if (val.indexOf('|') >= 0 || (VALID_VISIBILITY_VALUES.indexOf('|' + val + '|')
                        == -1)) {
    ...
    private Matrix parseTransformList(String val) throws SAXException {
        ...
        while (!scan.empty()) {
            ...
            if (cmd == null) {
                throw new SAXException(
                        "Bad transform function encountered in transform list: " + val);
            }
    ...
    */
    protected static Length parseLength(String val) throws SAXException {
        ...
        if (lastChar == '%') {
            ...
            unit = Unit.percent;
        } else if (end > 2 && Character.isLetter(lastChar) && Character
                .isLetter(val.charAt(end - 2))) {
            end -= 2;
    ...
    */
    private static SvgPaint parsePaintSpecifier(String val, String attrName) throws SAXException {
        if (val.startsWith("url(")) {
            ...
            if (closeBracket == -1) {
                throw new SAXException(
                        "Bad " + attrName + " attribute. Unterminated url() reference");
            }
    ...
    */
    ...
    private static void parseFont(Style style, String val) throws SAXException {
        ...
        if ("|caption|icon|menu|message-box|small-caption|status-bar|".indexOf('|' + val + '|')
                != -1) {
            return;
        }
    ...
    private static Length[] parseStrokeDashArray(String val) throws SAXException {
        ...
        if (dash.isNegative()) {
            throw new SAXException(
                    "Invalid stroke-dasharray. Dash segemnts cannot be negative: " + val);
        }
        ...
        while (!scan.empty()) {
            ...
            {
                throw new SAXException(
                        "Invalid stroke-dasharray. Non-Length content found: " + val);
            }
            if (dash.isNegative()) {
                throw new SAXException(
                        "Invalid stroke-dasharray. Dash segemnts cannot be negative: " + val);
    ...
    private static SVG.PathDefinition parsePath(String val) throws SAXException {
        ...
        float lastControlX = 0f, lastControlY
                = 0f;  // Last control point of the just completed bezier curve.
    ...
    private static String parseFunctionalIRI(String val, String attrName) throws SAXException {
        ...
        if (!val.startsWith("url(") || !val.endsWith(")")) {
            throw new SAXException(
                    "Bad " + attrName + " attribute. Expected \"none\" or \"url()\" format");
        }
