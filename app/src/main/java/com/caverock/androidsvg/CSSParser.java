...
public class CSSParser {
   ...
   */
        ...
        public boolean nextSimpleSelector(Selector selector) throws SAXException {
            ...
            while (!empty()) {
                ...
                if (consume('[')) {
                    ...
                    selectorPart
                            .addAttrib(attrName, (op == null) ? AttribOp.EXISTS : op, attrValue);
                    selector.addedAttributeOrPseudo();
                    continue;
                }
            ...
            if (scan.consume('!')) {
                ...
                if (!scan.consume("important")) {
                    throw new SAXException(
                            "Malformed rule set in <style> element: found unexpected '!'");
                }
    ...
    private static boolean ruleMatch(Selector selector, int selPartPos,
            List<SvgContainer> ancestors, int ancestorsPos, SvgElementBase obj) {
        // We start at the last part of the selector and loop back through the parts
        // Get the next selector part
        ...
        } else //if (sel.combinator == Combinator.FOLLOWS)
        {
            ...
            SvgElementBase prevSibling = (SvgElementBase) obj.parent.getChildren()
                    .get(childPos - 1);
    ...
    private static int getChildPosition(List<SvgContainer> ancestors, int ancestorsPos,
        ...
        if (ancestors.get(ancestorsPos)
                != obj.parent)  // parent doesn't match, so obj must be an indirect reference (eg. from a <use>)
        {
            return -1;
        }
        int childPos = 0;
